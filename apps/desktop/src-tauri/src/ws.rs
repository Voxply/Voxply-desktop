...
pub(crate) async fn spawn_ws_task(
    ...
    // A clone moved into the receive loop below, so voice-key handlers
    // (spawned off the loop to do async HTTP/HKDF work) can feed a built
    // `VoiceKeyOffer` back through the same outbound-JSON path as every
    // other WsCommand, without threading a second channel through.
            ...
            tokio::select! {
                        ...
                        Some(Ok(WsMessage::Text(text))) => {
                            if let Ok(server_msg) = serde_json::from_str::<WsServerMessage>(&text) {
                                match server_msg {
                                    ...
                                    WsServerMessage::VoiceJoined { channel_id, participants, voice_token, voice_wt_url, voice_cert_hash } => {
                                        // Open the WebTransport voice session now that the hub
                                        // has handed over the URL/token/cert hash
                                        // (voice-transport-v2.md). Spawned so the WS receive
                                        // loop is never blocked on the QUIC handshake -- mirrors
                                        // the old register_udp hand-off via a shared Arc, but the
                                        // Arc now holds the live connection itself.
                                        let session_arcs = {
                                            let app_state = app.state::<AppState>();
                                            let lock = app_state.voice.lock().unwrap();
                                            lock.as_ref()
                                                .filter(|s| s.channel_id == channel_id)
                                                .map(|s| (s.transport.clone(), s.voice_keys.clone()))
                                        };
                                        if let Some((transport_arc, voice_keys)) = session_arcs {
                                            let connect_url = voice_wt_url.clone();
                                            let connect_token = voice_token.clone();
                                            let connect_cert_hash = voice_cert_hash.clone();
                                            tokio::spawn(async move {
                                                match wavvon_voice::transport::VoiceTransport::connect(&connect_url, &connect_token, connect_cert_hash.as_deref()).await {
                                                    Ok(t) => *transport_arc.write().await = Some(std::sync::Arc::new(t)),
                                                    Err(e) => tracing::error!("voice WT connect failed: {e}"),
                                                }
                                            });

                                            // V4 voice encryption: offer our sender key (already
                                            // generated by voice_join before the WS join) to
                                            // everyone already in the channel.
                                            let cmd_tx_join = cmd_tx_for_task.clone();
                                            let hub_url_join = hub_url.clone();
                                            let token_join = token.clone();
                                            let channel_id_join = channel_id.clone();
                                            let participants_join = participants.clone();
                                            let http_client_join = app.state::<AppState>().http_client.clone();
                                            tokio::spawn(async move {
                                                let Some(gen) = voice_keys.read().await.own else { return };
                                                ...
                                                let recipients: Vec<String> = participants_join
                                                    .iter()
                                                ...
                                                let bundles = crate::voice_keys::build_offer_bundles(&http_client_join, &hub_url_join, &token_join, &identity, &channel_id_join, &gen, &recipients).await;
                                                if !bundles.is_empty() {
                                                    let _ = cmd_tx_join.send(WsCommand::VoiceKeyOffer { channel_id: channel_id_join, bundles });
                                                }
                                            });
                                        }
                                    ...
                                    WsServerMessage::VoiceParticipantLeft { channel_id, public_key } => {
                                        // V4 voice encryption: rotate our sender key and re-offer
                                        // it to everyone still in the channel, so the departed
                                        // member can't decrypt future audio
                                        // (voice-transport-v2.md "Leave" flow).
                                        let rotate_ctx = {
                                            let app_state = app.state::<AppState>();
                                            let lock = app_state.voice.lock().unwrap();
                                            lock.as_ref()
                                                .filter(|s| s.channel_id == channel_id)
                                                .map(|s| (s.voice_keys.clone(), s.roster_map.clone()))
                                        };
                                        if let Some((voice_keys, roster_map)) = rotate_ctx {
                                            let cmd_tx_leave = cmd_tx_for_task.clone();
                                            let hub_url_leave = hub_url.clone();
                                            let token_leave = token.clone();
                                            let channel_id_leave = channel_id.clone();
                                            let departed = public_key.clone();
                                            let http_client_leave = app.state::<AppState>().http_client.clone();
                                            tokio::spawn(async move {
                                                let next_key_id = voice_keys.read().await.own.map(|g| g.key_id + 1).unwrap_or(1);
                                                let gen = crate::voice_keys::generate_sender_key(next_key_id);
                                                voice_keys.write().await.set_own(gen);
                                                ...
                                                let recipients: Vec<String> = {
                                                    ...
                                                    rm.values().filter(|pk| *pk != &my_pubkey && *pk != &departed).cloned().collect()
                                                };
                                                if recipients.is_empty() {
                                                    return;
                                                }
                                                let bundles = crate::voice_keys::build_offer_bundles(&http_client_leave, &hub_url_leave, &token_leave, &identity, &channel_id_leave, &gen, &recipients).await;
                                                if !bundles.is_empty() {
                                                    let _ = cmd_tx_leave.send(WsCommand::VoiceKeyOffer { channel_id: channel_id_leave, bundles });
                                                }
                                            });
                                        }
                                    ...
                                    WsServerMessage::VoiceKeyReceived { channel_id, from_pubkey, ciphertext_hex, nonce_hex, .. } => {
                                        // V4 voice encryption: unwrap the sender's key and store
                                        // it for the pipeline's receive task to use once packets
                                        // arrive under this key_id.
                                        let ctx = {
                                            let app_state = app.state::<AppState>();
                                            let lock = app_state.voice.lock().unwrap();
                                            lock.as_ref().filter(|s| s.channel_id == channel_id).map(|s| s.voice_keys.clone())
                                        };
                                        if let Some(voice_keys) = ctx {
                                            let hub_url_recv = hub_url.clone();
                                            let token_recv = token.clone();
                                            let http_client_recv = app.state::<AppState>().http_client.clone();
                                            let from_pubkey2 = from_pubkey.clone();
                                            tokio::spawn(async move {
                                                let Ok(identity_path) = crate::identity::Identity::default_path() else { return };
                                                let Ok(identity) = crate::identity::Identity::load(&identity_path) else { return };
                                                ...
                                                let sender_pub = x25519_dalek::PublicKey::from(dh_arr);
                                                if let Ok(gen) = crate::voice_keys::voice_key_unwrap(&my_dh_sec, &sender_pub, &channel_id, &ciphertext_hex, &nonce_hex) {
                                                    voice_keys.write().await.insert_remote(&from_pubkey2, gen);
                                                }
                                            });
                                        }
                                    }
                                    WsServerMessage::VoiceKeyRequest { channel_id, new_pubkey, .. } => {
                                        // V4 voice encryption: a new participant joined and needs
                                        // our current sender key -- offer it directly.
                                        let ctx = {
                                            let app_state = app.state::<AppState>();
                                            let lock = app_state.voice.lock().unwrap();
                                            lock.as_ref().filter(|s| s.channel_id == channel_id).map(|s| s.voice_keys.clone())
                                        ...
                                        if let Some(voice_keys) = ctx {
                                            let cmd_tx_req = cmd_tx_for_task.clone();
                                            let hub_url_req = hub_url.clone();
                                            let token_req = token.clone();
                                            let channel_id_req = channel_id.clone();
                                            let new_pubkey2 = new_pubkey.clone();
                                            let http_client_req = app.state::<AppState>().http_client.clone();
                                            tokio::spawn(async move {
                                                let Some(gen) = voice_keys.read().await.own else { return };
                                                let Ok(identity_path) = crate::identity::Identity::default_path() else { return };
                                                let Ok(identity) = crate::identity::Identity::load(&identity_path) else { return };
                                                let bundles = crate::voice_keys::build_offer_bundles(&http_client_req, &hub_url_req, &token_req, &identity, &channel_id_req, &gen, std::slice::from_ref(&new_pubkey2)).await;
                                                if !bundles.is_empty() {
                                                    let _ = cmd_tx_req.send(WsCommand::VoiceKeyOffer { channel_id: channel_id_req, bundles });
                                                }
                                            });
                                        }
                                    }
                ...
                Some(cmd) = cmd_rx.recv() => {
                    let json = match cmd {
                        ...
                        WsCommand::VoiceJoin { channel_id } => {
                            serde_json::json!({ "type": "voice_join", "channel_id": channel_id })
                        }
                        ...
                        WsCommand::VoiceKeyOffer { channel_id, bundles } => {
                            serde_json::json!({
