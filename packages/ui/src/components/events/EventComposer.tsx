...
interface Props {
  ...
  canHubWide: boolean;
  /** Gates the hub-wide scope/announcement-channel/propagate/reminder/slots
   *  fields (events.md §5/§6/§7.5 "Phase 3") — omit on a platform whose
   *  create-event command doesn't forward these fields yet. */
  advancedFieldsSupported?: boolean;
  onSubmit: (payload: CreateEventPayload) => Promise<HubEvent>;
  onCreated: () => void;
  onClose: () => void;
}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await onSubmit({
        ...
        reminder_minutes: advancedFieldsSupported ? reminderOffsetToMinutes(reminderOffset) : null,
        slots: advancedFieldsSupported
          ? namedSlots.map((s) => ({
              name: s.name.trim(),
              capacity: s.capacity.trim() ? Number(s.capacity) : undefined,
            }))
          : [],
        hub_wide: advancedFieldsSupported && scope === "hub_wide",
        propagate_to_children: advancedFieldsSupported && anchorHasDescendants && propagateToChildren,
      });
      onCreated();
      onClose();
    } catch (e) {
